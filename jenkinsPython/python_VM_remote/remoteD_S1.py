      ...
      for x in range(0, len(vmList)):
          for vm in vmList:
              if vm.name in VmName[x] and vm.runtime.powerState in 'poweredOff':
                  try:
                    print VmName[x]
                    r = requests.get("http://192.168.0.14:7777/?Command=DeleteSnep&vm_name=" + VmName[x]+"&snapshot_name=" +snap)
                    #print "http://192.168.0.14:7777/?Command=DeleteSnep&vm_name=" + VmName[x]+"&snapshot_name=" +snap
                    r.text
                  except:
