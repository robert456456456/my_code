...
#print "SELECT id FROM test where `Name` = '" + test_name + "' and `Type` = 'Test'"
...
try:
    ...
    for row in cur.fetchall():
     bob.append(row[0])
     #print "vm name: " + row[0] + "; snap name: " + row[1]
          ...
          for vm in vmList:
              if vm.name in VmName[x]:
                  try:
                     ...
                     print VmName[x]
                    # print row[1]
                     r = requests.get("http://192.168.0.14:7777/?Command=RevertToSnapshot&vm_name=" + VmName[x] +"&snapshot_name="+row[1])
                     r.text
